...
def test_login_success() -> None:
    """Login OK → 200 + tokens + user."""
    with patch(
        "app.routers.auth.auth_service.login",
        new_callable=AsyncMock,
        return_value=VALID_TOKEN_RESPONSE,
    ):
...
def test_refresh_success() -> None:
    """Refresh OK → 200 + nouveaux tokens."""
    with patch(
        "app.routers.auth.auth_service.refresh",
        new_callable=AsyncMock,
        return_value=VALID_REFRESH_RESPONSE,
    ):
        with TestClient(app) as client:
            resp = client.post(
                "/auth/refresh",
