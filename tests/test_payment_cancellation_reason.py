"""Le motif d'une correction de versement, et ce qu'il vaut.
...
La même règle sert les deux corrections — annuler le versement, et déplacer
une de ses imputations vers le bon frais : la question posée est la même, et
elle n'a qu'une réponse.
...
from app.services.payments._correction import MOTIF_MAXIMUM, motif_valide


def test_un_motif_ecrit_est_conserve_tel_quel() -> None:
    assert (
        motif_valide("Montant saisi en double, la caisse ne contient que 5 000 F.")
...
def test_un_motif_tres_long_est_borne_a_la_colonne() -> None:
    ...
    """
    resultat = motif_valide("Annulation " + "x" * 900)
